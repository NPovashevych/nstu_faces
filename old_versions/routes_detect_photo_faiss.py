...
def is_identifiable_analysis(category: DBFaceCategory, analysis: dict) -> bool:
    ...
    return best_clip_category == CATEGORY_IDENTIFIABLE and best_clip_score >= 0.50
...
def get_face_color(category: DBFaceCategory | None, person: DBPerson | None) -> str:

    category_name = category.name if category else None
    category_code = category.code if category else None

    if category_name in {"low_quality", "unidentifiable"} or (category_code == "human" and category_name == "unidentifiable"):
        return "gray"

    if category_name in {"non_human", "artificial", "ai_generated", "uncertain"}:
        return "orange"
    ...
    if person_status in {PersonStatus.public.value, PersonStatus.non_public.value}:
        return "green"
...
def make_face_payload(face_index: int, face, category, person: DBPerson | None, category_score, quality, det_score, confidence, distance, analysis):
...
def process_single_face(db, pil_image, img, face, face_index, face_categories, clip_bundle=None, create_unknown: bool = False):
    bbox = face.bbox.astype(float).tolist()
    ...
    if quality < LOW_QUALITY_THRESHOLD:
        ...
        service_person = get_or_create_service_cluster_person(db, category.name) if create_unknown else None
    ...
    if is_identifiable_analysis(category=category, analysis=analysis):
        if create_unknown:
            person, cluster_dist = find_or_create_unknown_cluster_person(db=db, embedding=emb)
        else:
            person, cluster_dist = find_existing_unknown_cluster_person(db=db, embedding=emb)
...
def analyze_photo_faces(db: Session, file_bytes: bytes, create_unknown: bool = False):
    ...
    for index, face in enumerate(faces, start=1):
        ...
        if clip_bundle is None and result["status"] not in {"known", "low_det_score", "low_quality"}:
            clip_bundle = get_clip()

        results.append(result)

    return {"pil_image": pil_image, "img": img, "faces": faces, "results": results}
...
def save_detected_faces_to_db(db: Session, user: DBUser, file, file_bytes: bytes, analysis_result: dict):
    saved_image_path = save_file_as_image(file_bytes=file_bytes, original_filename=(file.filename or "upload.jpg"))
    ...
    try:
        for result in analysis_result["results"]:
            ...
            if status == "low_det_score":
                skipped_faces.append({"face_index": result["payload"]["face_index"], "reason": "low_det_score", "det_score": result["det_score"]})
                continue
            ...
            if person is None:
                if is_identifiable_analysis(category=category, analysis=result["analysis"]):
                    person, cluster_dist = find_or_create_unknown_cluster_person(db=db, embedding=result["embedding"])
                    result["person"] = person
                    result["distance"] = round(cluster_dist, 4)  if cluster_dist is not None else None
                else:
                    person = get_or_create_service_cluster_person(db=db, category_name=category.name)
                    result["person"] = person
            ...
            if get_person_status_value(person) == PersonStatus.unknown.value and is_identifiable_analysis(category=category, analysis=result["analysis"]):
                UNKNOWN_FACE_INDEX.add(embedding=result["embedding"], person_id=person.id)
...
def make_readonly_detect_response(db: Session, user: DBUser, file, analysis_result: dict):
...
@router.post("/faces")
def detect_faces_on_photo(file: UploadFile = File(...), user_id: int = Form(...), db: Session = Depends(get_db)):
    ...
    analysis_result = analyze_photo_faces(db=db, file_bytes=file_bytes, create_unknown=False)

    return make_readonly_detect_response(db=db, user=user, file=file, analysis_result=analysis_result)


@router.post("/faces/add-to-db")
def add_detected_photo_faces_to_db(file: UploadFile = File(...), user_id: int = Form(...), db: Session = Depends(get_db)):
    user = db.query(DBUser).filter(DBUser.id == user_id).first()
    ...
    if not can_save_uploaded_media(user):
        raise HTTPException(status_code=403, detail=("Only developer can add detected photo faces to database"))

    file_bytes = read_upload_file(file)

    analysis_result = analyze_photo_faces(db=db, file_bytes=file_bytes, create_unknown=True)

    return save_detected_faces_to_db(db=db, user=user, file=file, file_bytes=file_bytes,analysis_result=analysis_result)


@router.get("/person/{person_id}")
def get_detected_person_archive(person_id: int, db: Session = Depends(get_db)):
