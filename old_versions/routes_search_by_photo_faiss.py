...
from db.enums import EmbeddingType, FaceGender, IterationStatus, MediaType, PersonStatus, UserRole
from db.models import DBFaceCategory, DBFreeze, DBMedia, DBPerson, DBSource, DBUser
...
from commons.commons_base import normalize
from commons.common_search import build_person_result
...
def get_category(face_categories: dict, category_name: str) -> DBFaceCategory:
    ...
    raise RuntimeError(f"Face category '{category_name}' not found and fallback '{DEFAULT_FACE_CATEGORY}' not found.")


def is_identifiable_category(category: DBFaceCategory) -> bool:
    return category.name == CATEGORY_IDENTIFIABLE or category.code == CATEGORY_IDENTIFIABLE


def get_or_create_user_upload_source(db: Session) -> DBSource:
    source = db.query(DBSource).filter(DBSource.name == USER_UPLOAD_SOURCE_NAME).first()
...
def create_unknown_cluster_person(db: Session):
    ...
    person_create = PersonsCreate(name=cluster_tag, q_code=None, link=None, status=PersonStatus.unknown)
...
def get_or_create_service_cluster_person(db: Session, category_name: str):
    ...
    person_create = PersonsCreate(name=cluster_tag, q_code=None, link=None, status=PersonStatus.suspicious)
...
def find_existing_unknown_cluster_person(db: Session, embedding):
    ...
    person_id, best_dist = UNKNOWN_FACE_INDEX.find_best_match(embedding)
    ...
    db_person = db.query(DBPerson).filter(DBPerson.id == person_id).first()

    if not db_person:
        logging.warning("Unknown FAISS returned missing person_id=%s", person_id)
        return None, None
...
def create_uploaded_media_freeze_iteration(db: Session, user: DBUser, image_path: Path):
    ...
    freeze = DBFreeze(time_in=0.0, time_out=0.0, freeze_path=str(image_path), media_id=media.id)
...
def create_embedding_and_face(db, freeze, iteration_id, face, emb, person_id, distance, category, category_score, quality, gender, confidence, analysis):
...
def empty_person_result():
    return {"person": None, "summary": {"faces_count": 0, "frames_count": 0, "media_count": 0}, "medias": []}


def make_uploaded_preview(image_path: Path | None):
    if not image_path:
        return None

    return {"image_url": make_user_upload_url(str(image_path)), "path": str(image_path)}
...
def is_identifiable_clip_result(category: DBFaceCategory, analysis_result: dict) -> bool:

    clip = analysis_result.get("analysis", {}).get("clip", {})

    return (
        category.name == CATEGORY_IDENTIFIABLE or category.code == CATEGORY_IDENTIFIABLE
        or (clip.get("best_clip_category") == CATEGORY_IDENTIFIABLE and float(clip.get("best_clip_score") or 0.0) >= 0.50)
    )


def analyze_photo(db: Session, file_bytes: bytes):
    pil_image, img = read_image_from_upload(file_bytes)
    face_model = get_insightface()
    faces = face_model.get(img)

    if len(faces) == 0:
        return {"status": "no_faces", "pil_image": pil_image, "img": img, "faces": faces}

    if len(faces) > 1:
        return {"status": "multiple_faces", "pil_image": pil_image, "img": img, "faces": faces}
    ...
    best_ref, best_dist = REFERENCE_FACE_INDEX.find_best_match(emb)
...
def make_readonly_response(db: Session, user: DBUser, file, analysis_result: dict):
    log_base = {
    ...
    if status == "no_faces":
        log_history(db, user.id, "search.photo", {**log_base, "result": "no_faces"})
    ...
    if status == "multiple_faces":
        log_history(db, user.id, "search.photo", {**log_base, "result": "multiple_faces", "faces_count": len(analysis_result["faces"])})

        return {
            "mode": "multiple_faces",
            "message": ("На фото знайдено кілька облич. Для такого зображення скористайтесь сервісом детекції облич на фото."),
            "faces_count": len(analysis_result["faces"]),
    ...
    if status == "low_det_score":
        log_history(db, user.id, "search.photo", {**log_base, "result": "low_det_score", "det_score": analysis_result["det_score"], "bbox": analysis_result["bbox"]})

        return {
            "mode": "low_det_score",
            "message": ("Модель не впевнена, що на фото є людське обличчя."),
            "uploaded": None,
            "match": {"type": "low_det_score", "det_score": analysis_result["det_score"], "bbox": analysis_result["bbox"]},
            "result": None,
        }

    if status == "low_quality":
        log_history(db, user.id, "search.photo", {**log_base, "result": "low_quality", "quality": analysis_result["quality"], "det_score": analysis_result["det_score"]})

        return {
            "mode": "low_quality",
            "message": ("Якість фотографії недостатня для надійної ідентифікації. Ідентифікація ускладнена або неможлива."),
            "uploaded": None,
            "match": {"type": "low_quality", "quality": analysis_result["quality"], "det_score": analysis_result["det_score"], "bbox": analysis_result["bbox"], "analysis": analysis_result["analysis"]}, "result": None}
    if status == "clip_result":
        category = analysis_result["category"]

        if is_identifiable_clip_result(category, analysis_result):
            person, cluster_dist = find_existing_unknown_cluster_person(db=db, embedding=analysis_result["emb"])
            ...
            if person:
                ...
                return {
                    ...
                    "message": ("Особа поки неідентифікована. Тимчасове ім’я: unknown-кластер."),
                    "uploaded": None,
                    "match": {
                        "type": "unknown",
                        "person_id": person.id,
            ...
            log_history(
                ...
                user.id,
                "search.photo",
                {**log_base, "result": "no_match", "category": category.name, "category_score": analysis_result["category_score"], "quality": analysis_result["quality"], "det_score": analysis_result["det_score"]},
            )
        ...
        return {
            ...
            "message": ("Обличчя не віднесене до категорії придатних для ідентифікації."),
            "uploaded": None,
            "match": {
    ...
    raise RuntimeError(f"Unknown analysis status: {status}")


def save_analysis_to_db(db: Session, user: DBUser, file, file_bytes: bytes, analysis_result: dict):
    status = analysis_result["status"]

    if status in {"no_faces", "multiple_faces", "low_det_score"}:
        return make_readonly_response(db, user, file, analysis_result)

    saved_image_path = save_file_as_image(file_bytes, file.filename or "upload.jpg")

    media, freeze, iteration = create_uploaded_media_freeze_iteration(db=db, user=user, image_path=saved_image_path)
    ...
    try:
        if status == "known":
            ...
            create_embedding_and_face(
                ...
                category_score=analysis_result["category_score"],
                quality=analysis_result["quality"],
                gender=map_gender(analysis_result["face"], category),
                confidence=analysis_result["confidence"],
                analysis=analysis_result["analysis"],
            )
            ...
            log_history(
                ...
                {**log_base, "result": "known", "person_id": person.id, "person_name": person.name},
            )

            return {
                "mode": "photo_person_result",
                "message": ("Персону знайдено в базі. Фото збережено в БД."),
                "uploaded": make_uploaded_preview(saved_image_path),
                "match": {
                    "type": "known",
                    "person_id": person.id,
                    "person_name": person.name,
        ...
        if status == "low_quality":
            ...
            service_person = get_or_create_service_cluster_person(db, category.name)
            ...
            log_history(
                ...
                user.id,
                "search.photo.add_to_db",
                {**log_base, "result": "low_quality", "service_person_id": service_person.id},
            )

            return {
                "mode": "low_quality",
                "message": ("Фото низької якості збережено у службовий кластер low_quality."),
                "uploaded": make_uploaded_preview(saved_image_path),
                "match": {
                    "type": "low_quality",
                    "quality": analysis_result["quality"],
                ...
                "result": build_person_result(db, service_person),
            }
        ...
        if status == "clip_result":
            ...
            if is_identifiable_clip_result(category, analysis_result):
                person, cluster_dist = find_or_create_unknown_cluster_person(db=db, embedding=analysis_result["emb"])
                distance_for_source = round(cluster_dist, 4) if cluster_dist is not None else None
                ...
                UNKNOWN_FACE_INDEX.add(embedding=analysis_result["emb"], person_id=person.id)

                logging.info("Unknown embedding added to FAISS: embedding_id=%s, person_id=%s, faiss_size=%s", detected_embedding.id, person.id, UNKNOWN_FACE_INDEX.size)
            ...
            log_history(
                ...
                user.id,
                "search.photo.add_to_db",
                {**log_base, "result": "service_category", "service_person_id": service_person.id, "category": category.name},
            )

            return {
                "mode": "service_category",
                "message": ("Фото збережено у службовий кластер."),
                "uploaded": make_uploaded_preview(saved_image_path),
                "match": {
                    "type": "service_category",
                    "category": category.name,
        ...
        raise RuntimeError(f"Cannot save status: {status}")
...
@router.post("/person")
def search_person_by_photo(file: UploadFile = File(...), user_id: int = Form(...), db: Session = Depends(get_db)):
    ...
    analysis_result = analyze_photo(db=db, file_bytes=file_bytes)

    return make_readonly_response(db=db, user=user, file=file, analysis_result=analysis_result)


@router.post("/person/add-to-db")
def add_photo_search_result_to_db(file: UploadFile = File(...), user_id: int = Form(...), db: Session = Depends(get_db)):
    user = db.query(DBUser).filter(DBUser.id == user_id).first()
    ...
    if not can_save_uploaded_media(user):
        raise HTTPException(status_code=403, detail=("Only developer can add uploaded photo to database"))
    ...
    return save_analysis_to_db(db=db, user=user, file=file, file_bytes=file_bytes, analysis_result=analysis_result)
