...
def get_url_file_name(url: str, content_type: str | None) -> str:
    parsed = urlparse(url)
...
def download_url(url: str, destination: Path) -> tuple[str, str | None]:
    headers = {
        "User-Agent": "NSTUFacesMediaAnalyzer/1.0 (media analysis service)"
    }

    try:
        with requests.get(url, stream=True, timeout=URL_TIMEOUT, allow_redirects=True, headers=headers) as response:
            response.raise_for_status()

            content_type = response.headers.get("Content-Type")
            file_name = get_url_file_name(str(response.url), content_type)
        ...
        raise HTTPException(status_code=400, detail=f"Не вдалося завантажити медіа за URL: {error}") from error
...
def analyze_image(db: Session, img, face_model, clip_model, clip_preprocess, clip_text_features, clip_prompt_categories, face_categories) -> list[dict]:
    ...
    return [normalize_detected_face(db=db, result=result) for result in results]


def build_image_response(source_type: str, file_name: str, faces: list[dict]) -> dict:
    return {
        "mode": "image",
...
def analyze_video(db: Session, video_path: Path, source_type: str, file_name: str, face_model, clip_model, clip_preprocess, clip_text_features, clip_prompt_categories, face_categories) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="nstu_detect_frames_") as frames_folder_value:
        frames_folder = Path(frames_folder_value)

        for frame_index, (time_in, time_out) in enumerate(scenes):
            freeze_path = frames_folder / f"frame_{frame_index:06d}.jpg"
...
async def detect_media(file: UploadFile | None = File(default=None), url: str | None = Form(default=None), db: Session = Depends(get_db)):
    if file is None and not url:
        raise HTTPException(status_code=400, detail="Передайте файл або URL.")

    if file is not None and url:
        raise HTTPException(status_code=400, detail="Передайте щось одне: файл або URL.")
    ...
    if file is not None:
        ...
        if media_type == "image":
            ...
            return build_image_response(source_type="file", file_name=file_name, faces=faces)
    ...
    with tempfile.TemporaryDirectory(prefix="nstu_detect_url_") as temp_folder_value:
        temporary_path = Path(temp_folder_value) / "downloaded_media"

        file_name, content_type = download_url(url=url.strip(), destination=temporary_path)
        media_type = detect_media_type(file_name=file_name, content_type=content_type)

        suffix = Path(file_name).suffix.lower()

        if suffix:
            media_path = temporary_path.with_suffix(suffix)
            temporary_path.replace(media_path)
        else:
            media_path = temporary_path

        if media_type == "image":
            img = load_image_from_path(media_path)
            faces = analyze_image(db=db, img=img, face_model=face_model, clip_model=clip_model, clip_preprocess=clip_preprocess, clip_text_features=clip_text_features, clip_prompt_categories=clip_prompt_categories, face_categories=face_categories)

            return build_image_response(source_type="url", file_name=file_name, faces=faces)

        return analyze_video(db=db, video_path=media_path, source_type="url", file_name=file_name, face_model=face_model, clip_model=clip_model, clip_preprocess=clip_preprocess, clip_text_features=clip_text_features, clip_prompt_categories=clip_prompt_categories, face_categories=face_categories)
