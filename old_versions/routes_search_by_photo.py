...
from db.models import DBEmbedding, DBFaceCategory, DBFreeze, DBIteration, DBMedia, DBPerson, DBSource, DBUser
...
SERVICE_NAME = "routes_search_by_photo.py"
...
def make_uploaded_preview(image_path: Path | None):
    ...
    return {
        "image_url": make_user_upload_url(str(image_path)),
        "path": str(image_path),
    }
...
def is_identifiable_clip_result(category: DBFaceCategory, analysis_result: dict) -> bool:
    ...
    return (
        ...
        or category.code == CATEGORY_IDENTIFIABLE
        or (
            clip.get("best_clip_category") == CATEGORY_IDENTIFIABLE
            and float(clip.get("best_clip_score") or 0.0) >= 0.50
        )
    )
...
def make_readonly_response(
    ...
    if status == "multiple_faces":
        ...
        return {
            ...
            "message": (
                ...
                "Для такого зображення скористайтесь сервісом детекції облич на фото."
            ),
            "faces_count": len(analysis_result["faces"]),
    ...
    if status == "low_quality":
        ...
        return {
            "mode": "low_quality",
            "message": (
                "Якість фотографії недостатня для надійної ідентифікації. "
                "Ідентифікація ускладнена або неможлива."
            ),
            "uploaded": None,
            "match": {
                "type": "low_quality",
    ...
    try:
        if status == "known":
            ...
            update_iteration(
                db,
                iteration.id,
                IterationUpdate(status=IterationStatus.completed, finished_at=datetime.now()),
            )
        ...
        if status == "clip_result":
            ...
            if is_identifiable_clip_result(category, analysis_result):
                ...
                return {
                    "mode": "photo_unknown_result",
                    "message": "Фото збережено в unknown-кластер.",
                    "uploaded": make_uploaded_preview(saved_image_path),
                    "match": {
                        "type": "unknown",
                        "person_id": person.id,
...
def add_photo_search_result_to_db(
    ...
    if not can_save_uploaded_media(user):
        raise HTTPException(
            status_code=403,
            detail="Only developer can add uploaded photo to database",
        )
