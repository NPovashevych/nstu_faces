...
from services.config import (
    ...
    HIRES_INTVNEWS_CATALOG,
    HIRES_DIGITAL_CATALOG,
...
LOG_PATH = Path("../logs/create_hires_catalog.log")
...
def normalize_name(name: str) -> str:
    """
    Нормалізація назви для пошуку дублікатів.
...
def get_year_folder(
    ...
    """
    Повертає лише папку конкретного дозволеного року.

    Інші папки в корені архіву навіть не переглядаються.
    """
...
def scan_year_folder(
    ...
    """
    Сканує одну папку року та повертає річний каталог.
    ...
    try:
        for file_path in year_folder.rglob("*"):
            try:
                ...
                # Захист від випадкового збігу сформованого ключа
                ...
                if duplicate_number == 0:
                    ...
                    logging.warning(
                        f"Duplicate MXF name: {original_name}"
                    )
                    logging.warning(
                        f"  duplicate number: "
                        f"{duplicate_number}"
...
def build_hires_catalog(
    ...
    """
    Формує окремі JSON-каталоги для кожного дозволеного року.
    """
    logging.info("================================")
    ...
    for year in sorted(years):
        ...
        if year_folder is None:
            # Порожній JSON показує, що цей рік був передбачений,
            # але відповідної папки немає.
            save_json(
...
if __name__ == "__main__":
    ...
    build_hires_catalog(
        ...
        years=HIRES_INTVNEWS_YEARS,
        catalog_folder=HIRES_INTVNEWS_CATALOG,
    ...
    build_hires_catalog(
        ...
        years=HIRES_DIGITAL_YEARS,
        catalog_folder=HIRES_DIGITAL_CATALOG,
