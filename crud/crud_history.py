...
def get_histories_by_user(db: Session, user_id: int):
    return db.query(DBHistory).filter(DBHistory.user_id == user_id).order_by(DBHistory.created_at.desc()).all()


def create_history(db: Session, history: HistoryCreate):
    db_history = DBHistory(action=history.action, user_id=history.user_id)
